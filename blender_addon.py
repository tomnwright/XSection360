...
class Setup:
    ...
    def apply_setup():
        ...
        """
        mat = Setup.create_flat_mat('WHITE', (1, 1, 1, 1))
        Setup.set_world_bg((0, 0, 0, 1))

        view_layer = bpy.context.view_layer
        Setup.exclude_all_collections(view_layer)

        workingCol = Setup.copy_mesh_to_new_collection('Output', bpy.data.collections[0], mat)

        Setup.hide_all_without_collection()

        Setup.config_render_settings()
        Setup.configure_viewer_node()

        Setup.create_camera('Cam 360', workingCol)
